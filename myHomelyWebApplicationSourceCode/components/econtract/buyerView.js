...
export default function EContractModal({ state, setState, data }) {
  ...
  const onSubmitCounter = (values) => {
    ...
    axios
      .post(
        `${process.env.NEXT_PUBLIC_SERVER_API}/api/econtract/sellerCounterOffer`,
        {
          id: data._id,
          data: values,
        },
  ...
  return (
    ...
    >
      <Formik
        enableReinitialize
        initialValues={
          data?.sellerSubmissions?.[data?.sellerSubmissions?.length - 1]
        }
                  ...
                  >
                    <div className={classes.flexGap}>
                      ...
                      <div className={classes.title}>
                        ...
                        {data?.sellerSubmissions?.[
                          data?.sellerSubmissions?.length - 1
                        ].OfferDetails.loan
                          ? " loan"
                          : " cash"}
                      </div>
