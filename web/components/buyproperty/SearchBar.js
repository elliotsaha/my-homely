...
export default function SearchBar({
  ...
  useEffect(() => {
    axios
      ...
      .then((res) => {
        ...
        setPaginationTotal(res.data.pagination);
      })
      .catch((err) => {
        console.log(err);
      });
  }, [rawBuyerInput, filterValues, activePage, query]);
  ...
  useEffect(() => {
    if (Object.keys(query).length !== 0) {
      setRawBuyerInput(query.query);
      setOpenModal(buyerData[0]?._id);
    }
  }, [query, buyerData]);
